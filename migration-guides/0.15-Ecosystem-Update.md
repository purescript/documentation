...
Breaking changes:
- Remove deprecated `exists` ([#35](https://github.com/purescript-node/purescript-node-fs/pull/61) by @sigma-andex). Use `Sync.exists` from [`purescript-node-fs`](https://github.com/purescript-node/purescript-node-fs) instead.
