...
### Remove `Async.exists` (`purescript-node-fs`/`purescript-node-fs-aff`)

The async `exists` version from `purescript-node-fs` and the corresponding version using `Aff` in `purescript-node-fs-aff` have been removed since the underlying Node.js version has been deprecated since Node.js version 1.

**To fix**:
- use `exists` from the `Sync` module
- to use in `Aff` context use `liftEffect (Sync.exists path)`
